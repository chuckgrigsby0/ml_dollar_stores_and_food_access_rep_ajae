...
table_att_comb <- bind_cols(table_att, select(table_att_rf, Urban, Rural))
...
kable(table_att_comb, 
      ...
      caption = paste0("Average Treatment Effects of Dollar Store Entry on Low Food Access Status Using Random Forest and XGBoost, 
                        Block Groups Receiving a Dollar Store from 2006-2020")) %>% 
  kable_styling() %>%
  add_header_above(c('', 'XGBoost (Main Results)' = 2, 'Random Forest' = 2)) %>%
