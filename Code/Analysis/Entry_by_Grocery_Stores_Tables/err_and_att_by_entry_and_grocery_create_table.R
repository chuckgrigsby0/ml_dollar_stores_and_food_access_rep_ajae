...
emp_res_format <- emp_res %>%
  ...
  # Round all other numeric columns to 4 digits
...
# rows_per_group <- 9
# unique_entries <- seq_along(get_lvl_ds)
# 
# # Create list of grouping specifications
# group_specs <- map2(get_lvl_ds, unique_entries, function(.x, .y) {
#   start_row <- (.y - 1) * rows_per_group + 1
#   end_row <- .y * rows_per_group
#   
#   list(
#     label = .x,
#     start = start_row,
...
# })
# 
# # Now apply each group specification to the table
# tbl_fnl <- reduce(
#   .x = group_specs,  # The list to iterate over
#   .f = function(tbl_acc, group_info) {  # group_info is each element from group_specs
#     pack_rows(tbl_acc, 
#               group_label = group_info$label, 
#               start_row = group_info$start, 
#               end_row = group_info$end, 
#               latex_align = 'l')
#   },
#   .init = tbl  # Start with the original table
# )
# 
# tbl_fnl
# boot_res_err <- err_and_att %>% 
#   
#   map_dfr(function(.x){ 
#     
#     .x %>% pluck('entry_events_bins', 'err_by_entry_grocery')
#     
#   })
# # -------------------------------------------------------------------------------------------- #
# boot_sd_err <- boot_res_err %>%
#   
#   filter(
#     (!is.na(pct_err_avg_pre) & !is.infinite(pct_err_avg_pre) )
#   ) %>%
#   
#   group_by(entry_events_bins, Grocery_Count_10mile_2005_bins) %>%
#   
#   summarise(across(
#     .cols = c(err_avg_pre, pct_err_avg_pre), 
#     .fns = list('sd' = \(x) sd(x, na.rm = TRUE)), 
#     .names = '{.col}_{.fn}'
#   ) )
# 
# sd_vars <- names(boot_sd_err) %>% str_subset(pattern = 'sd$')
# # -------------------------------------------------------------------------------------------- #
# emp_res_err <- pluck(err_and_att, 
#                  1, 'entry_events_bins', 'err_by_entry_grocery') %>%
#   
#   left_join(boot_sd_err, by = c('entry_events_bins', 'Grocery_Count_10mile_2005_bins')) %>%
#   
#   select(entry_events_bins, Grocery_Count_10mile_2005_bins, 
#          err_avg_pre, err_avg_pre_sd,  
#          pct_err_avg_pre, pct_err_avg_pre_sd)
# # -------------------------------------------------------------------------------------------- #
# 
# # -------------------------------------------------------------------------------------------- #
# emp_res_err_wd <- emp_res_err %>%
#   
#   filter(
#     (Grocery_Count_10mile_2005_bins %in% get_lvl_groc) & 
#       (entry_events_bins %in% get_lvl_ds) 
#   ) %>%
#   
#   pivot_longer(cols = err_avg_pre:pct_err_avg_pre_sd, 
#                names_to = 'variable', 
#                values_to = 'value') %>%
#   
#   pivot_wider(names_from = Grocery_Count_10mile_2005_bins, 
