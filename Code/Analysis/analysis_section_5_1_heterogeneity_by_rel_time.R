...
# pnorm(q, mean = 0, sd = 1, lower.tail = TRUE, log.p = FALSE) --> gives the probability that q is less than or equal q.
# qnorm(p, mean = 0, sd = 1, lower.tail = TRUE, log.p = FALSE) --> gives the critical value for a given percentile.

# ci_function <- function(df, pctl, pct_name){ 
#   
#   lower <- df$estimate - qnorm(pctl)*df$estimate_sd
#   upper <- df$estimate + qnorm(pctl)*df$estimate_sd
#   
#   new_df <- data.frame(lower, upper)  
#   
#   colnames(new_df) <- paste0(colnames(new_df), '_', pct_name)
#   
#   return(new_df)
#   
# }
# cis <- c(0.995, 0.975, 0.95)
#ci_names <- c('1_pct', '5_pct', '10_pct')
#ci_df <- map2_dfc(cis, ci_names, function(.x, .y) ci_function(df = rel_year_heterogeneity_emp, 
#                                                              pctl = .x, 
#                                                              pct_name = .y) )
