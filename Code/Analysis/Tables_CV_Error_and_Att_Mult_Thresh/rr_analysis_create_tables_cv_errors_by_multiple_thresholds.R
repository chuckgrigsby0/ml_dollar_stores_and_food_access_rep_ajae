# To run this program, simply change from Urban to Rural and vice-versa to create and save a table of CV errors
...
# Subset the ATTs
# -------------------------------------------------------------------------------------------- # 
boot_cv_errors <- boot_data %>%
  
  map_dfr(function(.x){ 
    
    att_dta <- .x %>% pluck('cv_errors_summary')
    
  })
# -------------------------------------------------------------------------------------------- # 
# Combine the empirical ATTs with the bootstrapped ATTs
# -------------------------------------------------------------------------------------------- # 
boot_cv_errors <- bind_rows(emp_errors_and_preds$cv_errors_summary, boot_cv_errors)
# -------------------------------------------------------------------------------------------- # 

# Obtain a string vector for the measures requiring summary statistics. 

summary_stat_vars <- str_subset(names(boot_cv_errors), '^err|^preds|^actual|^pct_err'); summary_stat_vars

# Compute Bootstrapped SDs. 
summary_boot_cv_errors <- boot_cv_errors %>% 
...
emp_cv_errors <- emp_errors_and_preds %>% 
  pluck('cv_errors_summary') %>% 
  bind_cols(summary_boot_cv_errors) %>%
  select(sort(names(.) ) )

correct_col_order = c('actual_avg', 'preds_orig_avg', 'err_orig_avg', 'err_orig_avg_sd', 'pct_err_orig', 'pct_err_orig_sd', 
                      'preds_f1_avg', 'err_f1_avg', 'err_f1_avg_sd', 'pct_err_f1', 'pct_err_f1_sd',
                      'preds_lipton_avg', 'err_lipton_avg', 'err_lipton_avg_sd', 'pct_err_lipton', 'pct_err_lipton_sd',
                      'preds_j_index_avg', 'err_j_index_avg', 'err_j_index_avg_sd', 'pct_err_j_index', 'pct_err_j_index_sd',
                      'preds_avg_avg', 'err_avg_avg', 'err_avg_avg_sd', 'pct_err_avg', 'pct_err_avg_sd')

emp_cv_errors_tidy <- emp_cv_errors %>% 
  select(-c('id', 'actual_avg_sd'), -matches('^preds_.*sd$') ) %>% 
...
groupings <- list(
  ...
  'Base' = c('actual_avg', 'preds_orig_avg', 
             'err_orig_avg', 'err_orig_avg_sd', 
             'pct_err_orig', 'pct_err_orig_sd'), 
  
  'J Index' = c('preds_j_index_avg', 'err_j_index_avg', 'err_j_index_avg_sd', 
                'pct_err_j_index', 'pct_err_j_index_sd'), 
  
  
  'Average' = c('preds_avg_avg', 'err_avg_avg', 'err_avg_avg_sd', 
                'pct_err_avg', 'pct_err_avg_sd'),
  
  
  'F1/2' = c('preds_lipton_avg', 'err_lipton_avg', 'err_lipton_avg_sd', 
             'pct_err_lipton', 'pct_err_lipton_sd'), 
  
  'F1' = c('preds_f1_avg', 'err_f1_avg', 'err_f1_avg_sd', 
           'pct_err_f1', 'pct_err_f1_sd') 
...
emp_cv_errors_tidy <- emp_cv_errors_tidy %>%
...
emp_cv_errors_tidy <- emp_cv_errors_tidy %>% 
  ...
  separate_wider_regex(cols = temp, 
                       patterns = c(Measure = '^actual|^preds|^err|^pct_err', 
                                    '.*', 
                                    Statistic = 'avg$|sd$'), 
                       too_few = 'align_start') %>%
  mutate(
...
emp_cv_errors_tidy_ci <- emp_cv_errors_tidy %>%
  ...
  mutate(across(.cols = where(is.numeric), 
                .fn = \(x) case_when(Measure == 'CV Error (%)' ~ x * 100, 
                                     TRUE ~ x) ) 
  ) %>%
  mutate(
...
emp_cv_errors_tidy_ci <- bind_rows(table_opt_thresh, emp_cv_errors_tidy_ci)

target_dir <- paste0('Analysis/Tables/Low_Access/', model_geography); target_dir
saveRDS(emp_cv_errors_tidy_ci, file = here::here(target_dir,
                                               paste0(str_to_lower(model_geography), 
                                                      '_cv_errors_w_multiple_thresholds', '.rds')))
rm(list=ls())
gc()
