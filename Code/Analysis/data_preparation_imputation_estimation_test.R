...
subset_data <- function(dta){ 
  
  dta_sampled <- mc_cv(dta, prop = 0.05, times = 1)
  
  dta_sampled <- training(dta_sampled$splits[[1]])
...
# Pre-Treatment
dta_untreated <- subset_data(dta = dta_untreated)
dta_untreated_non_model_vars <- subset_data(dta = dta_untreated_non_model_vars)
# Treated
dta_treated <- subset_data(dta = dta_treated)
dta_treated_non_model_vars <- subset_data(dta = dta_treated_non_model_vars)
...
rm(list=ls()[!(ls() %in% c('model_vars', 'non_model_vars', 'acs_covars', 'econ_geog_vars',
                           ...
                           'ds_bans_stats_all_rural', 'ds_bans_stats_all_urban', 'param_id'))])
gc()
# -------------------------------------------------------------------------------------------- #