# Estimates bootstrap estimates of pre-treatment CV errors and post-treatment counterfactuals
# using data containing state-linear trends. Robustness check following reviewer comments. 
# ----------------------------------- #
# Load packages
# ----------------------------------- #
library(pacman)
...
# -------------------------------------------------------------------------------------------- #  
bootstrap_by_tracts = '_tracts_w_time_trends' # or NULL to bootstrap by block-group and stratify by relative time. 
...
class_counts <- dta_untreated_wfolds %>% 
  reframe(across(.cols = matches('^low_access'), .fns = table))
...
toc()
# -------------------------------------------------------------------------------------------- #
filename = paste0(str_to_lower(model_geography), '_', model_dep_var, '_', 'bootstrap_lhs_w_time_trends', '_', bootstrap_id, '.rds')
...
dir_path <- here::here('Analysis', 
                       paste0(model_geography, '_', 'Bootstrap'),
                       dir_model_dep_var,
                       paste0('bootstrap_01_499', bootstrap_by_tracts, '_lhs'))

# Create directory if it doesn't exist
dir.create(dir_path, recursive = TRUE, showWarnings = FALSE)

# Save the file
saveRDS(xgb_model_results, file.path(dir_path, filename))
# -------------------------------------------------------------------------------------------- #