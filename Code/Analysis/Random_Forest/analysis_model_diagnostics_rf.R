...
filename <- paste0('rf_10m_', str_to_lower(model_geography), '_', model_dep_var, '_training', '.rds')
...
treated_preds <- model_output$data_cf_preds %>%
  ...
  select(GEOID, year, event_year, rel_year, Geography, all_of(model_dep_var), pred_class_cf, tau) %>%
