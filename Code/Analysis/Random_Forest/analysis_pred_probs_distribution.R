...
summary_fit_metrics <- summary_fit_metrics %>% 
  mutate(uci_99 = .estimate + 2.576*.estimate_sd, 
         lci_99 = .estimate - 2.576*.estimate_sd)
...
names(overall_fit_metrics) <- names(overall_fit_metrics) %>% str_replace_all(pattern = tidy_cols); names(overall_fit_metrics)
