# Script to tune RF models to find optimal hyperparameter set, excluding ntrees. 
# -------------------------------------------------------------------------------------------- #  
# Load packages. 
# -------------------------------------------------------------------------------------------- #  
library(pacman)
...
#--------------------------------------------------------------------------------------------#
ntrees = 500 # This stays fixed while tuning other hyperparameters. 
#--------------------------------------------------------------------------------------------#
tuning_params <- list.files(path = here::here('Analysis', 'Model_Training', 'Low_Access', 'RF'), 
                            pattern = paste0('^lhs_.*', str_to_lower(model_geography) ) ) 
...
rf_results <- rf_tuning(param_idx = param_id)
...
# -------------------------------------------------------------------------------------------- #
filename = paste0('rf_10m_', str_to_lower(model_geography), '_', model_dep_var, '_tuning','_', param_id, '.rds')
