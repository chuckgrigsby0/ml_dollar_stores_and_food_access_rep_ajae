...
# summary_fit_metrics$metrics %>% filter(rel_year < 0 & .metric == 'sens') 
# -------------------------------------------------------------------------------------------- #
# Load bootstrap data. 
# -------------------------------------------------------------------------------------------- #
...
alpha_level <- 0.01
ci_threshold <- qnorm(1 - alpha_level/2)

summary_fit_metrics <- summary_fit_metrics %>% 
  mutate(uci_99 = .estimate + ci_threshold*.estimate_sd, 
         lci_99 = .estimate - ci_threshold*.estimate_sd)
...
names(overall_fit_metrics) <- names(overall_fit_metrics) %>% 
  str_replace_all(pattern = tidy_cols); names(overall_fit_metrics)

# ------------------------------------------------ #
# Save the data. 
# ------------------------------------------------ #
saveRDS(overall_fit_metrics, here::here('Analysis', 
                                        'Tables', 
                                        'Low_Access', 
                                        model_geography, 
...
# ------------------------------------------------ #
# Replicate the above but using relative treatment timing. 
# ------------------------------------------------ #
relative_time_fit_metrics <- summary_fit_metrics %>%
  select(-c(.estimate_avg)) %>%
  filter(!is.na(rel_year)) %>%
  mutate(across(.cols = where(is.numeric), 
                .fns = \(x) round(x, digits = 4))) %>%
...
relative_time_fit_metrics_est <- relative_time_fit_metrics %>% 
  pivot_longer(cols = c('.estimate', '.estimate_sd', 'uci_99', 'lci_99'), 
               values_to = 'values', 
               names_to = 'names') %>%
  filter(str_detect(string = names, pattern = '_99$', negate = TRUE)) %>% 
  filter(names == '.estimate') %>%
  pivot_wider(id_cols = 'rel_year', 
              names_from = '.metric', 
              values_from = 'values') %>%
  mutate(Statistic = 'Est') %>% relocate(Statistic)


relative_time_fit_metrics_sd <- relative_time_fit_metrics %>% 
  pivot_longer(cols = c('.estimate', '.estimate_sd', 'uci_99', 'lci_99'), 
               values_to = 'values', 
               names_to = 'names') %>%
  filter(str_detect(string = names, pattern = '_99$', negate = TRUE)) %>% 
  filter(names == '.estimate_sd') %>%
  pivot_wider(id_cols = 'rel_year', 
              names_from = '.metric', 
              values_from = 'values') %>%
  mutate(Statistic = 'SD') %>% relocate(Statistic)

transform_cols <- names(relative_time_fit_metrics_est)[-c(1, 2)]

relative_time_fit_metrics <- bind_rows(relative_time_fit_metrics_est, 
                  relative_time_fit_metrics_sd) %>% 
  arrange(desc(rel_year)) %>% 
  mutate(across(.cols = where(is.numeric), 
                .fns = \(x) as.character(x))) %>% 
  rowwise() %>% 
  mutate(across(.cols = all_of(transform_cols), 
                .fns = \(x) case_when(Statistic == 'SD' ~ paste0('(', x, ')'), 
                                      TRUE ~ x) ) )


saveRDS(relative_time_fit_metrics, here::here('Analysis', 
                                        'Tables', 
                                        'Low_Access', 
                                        model_geography, 
...
# Precision-Recall (ideal for imbalanced data)
# pr_curve_dta <- pr_curve(data = dta_auc_ut, 
#                          truth = actual, 
#                          pred_probs)
# 
# # Plot the relationship between recall/TPR/Sensitivity and precision (tp/(tp+fp))
# pr_curve_dta %>%
...
# 
# # Create ROC data
# roc_curve_dta <- roc_curve(data = dta_auc_ut, 
#                            truth = actual, 
#                            pred_probs)
# 
# # Plot the relationship between the FPR (1 - specificity) and sensitivity ()
# roc_curve_dta %>%
