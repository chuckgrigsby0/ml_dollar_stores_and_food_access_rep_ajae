# Script creates latex tables comparing pre-treatment average CV error and post-treatment ATTs across urban and rural areas using 
# state-by-year FEs and state, year FEs with linear time trends. 
# --------------------------------- # 
pacman::p_load('here', 'dplyr', 'purrr', 'stringr')
# --------------------------------- # 
rootname <- 'average_change_in_outcomes_relative_time_lhs_w_time_trends.rds'
urban_att_tt <- readRDS(here::here('Analysis', 'Tables', 'Low_Access', 'Urban', paste0('urban_', rootname) ) )
rural_att_tt <- readRDS(here::here('Analysis', 'Tables', 'Low_Access', 'Rural', paste0('rural_', rootname) ) )

att_tt <- bind_rows(urban_att_tt, rural_att_tt) %>% rename(estimate = estiamte)
...
source(here::here('Code', 'Analysis', 'rr_analysis_cv_error_and_att_create_table.R'))
table_att_comb <- bind_cols(table_att, select(table_att_tt, Urban, Rural))
...
kable(table_att_comb, 
      ...
      caption = paste0("Average Treatment Effects of Dollar Store Entry on Low Food Access Status Using State and Year Fixed Effects (FEs) 
                       and State Linear Year Trends, Block Groups Receiving a Dollar Store from 2006-2020")) %>% 
  kable_styling() %>%
  add_header_above(c('', 'State-x-Time FEs' = 2, 'State, Time, Linear-State Trends' = 2)) %>%
