# Script to create hyperparameter matrix using LHS for random forest models.  
# -------------------------------------------------------------------------------------------- #  
library(pacman)
p_load('furrr', 'fixest', 'xgboost', 'ranger', 'mlr3verse', 'mlr3tuningspaces', 'future', 'parallelly', 'tictoc', 'here')
# -------------------------------------------------------------------------------------------- #  
# Load and prepare data. 
# -------------------------------------------------------------------------------------------- #  
model_geography = 'Urban'; model_dep_var = 'low_access' # Adjust model_geography according to Urban/Rural. 
# -------------------------------------------------------------------------------------------- #  
...
class_ratios = class_ratios %>% filter(dep_var == model_dep_var) %>% pull(ratio)
#--------------------------------------------------------------------------------------------#
...
ntrees = 500
#--------------------------------------------------------------------------------------------#
n_predictors <- length(colnames(x_train))
#--------------------------------------------------------------------------------------------#

# Define LHS tuner
search_space <- ps(
  mtry = p_dbl(lower = 0, upper = n_predictors),
  sample.fraction = p_dbl(lower = 0.1, upper = 1),
  replace = p_lgl(),
  case.weights = p_dbl(lower = 1, upper = class_ratios)
)


default_ps <- data.frame(
...
set.seed(243444)
design <- generate_design_lhs(param_set = search_space, n = 49)

design <- as.data.table(design$data)

design <- bind_rows(default_ps, design) 

filename = paste0('lhs_hyperparameters_training_', str_to_lower(model_geography), '.rds')

saveRDS(design, file = here::here('Analysis', 'Model_Training', 'Low_Access', 'RF', filename) )