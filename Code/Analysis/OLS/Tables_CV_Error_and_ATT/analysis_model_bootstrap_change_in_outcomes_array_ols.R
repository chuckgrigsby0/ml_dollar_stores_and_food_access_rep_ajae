# Script to compute average CV errors and ATTs based on OLS-LPM using bootstrap procedure.  
# -------------------------------------------------------------------------------------------- #
...
# -------------------------------------------------------------------------------------------- #
source(here::here('Code', 'Analysis', 'OLS', 'data_preparation_imputation_estimation_ols.R'))
# -------------------------------------------------------------------------------------------- #
...
dir_bootstrap <- paste0('bootstrap_', bootstrap_ids, bootstrap_by_tracts, '_', 'ols') # NULL or '_tracts'
...
# -------------------------------------------------------------------------------------------- #
untreated_preds <- model_output$data_cv_preds %>% 
  
  left_join(dta_untreated_non_model_vars, by = c('GEOID', 'year')) %>%
  
  select(GEOID, year, event_year, rel_year, Geography, actual, preds) %>%
...
treated_preds <- model_output$data_cf_preds %>%
  ...
  select(GEOID, year, event_year, rel_year, Geography, actual, preds, tau) %>%
...
dir_bootstrap <- paste0('bootstrap_change_in_outcomes', bootstrap_by_tracts, '_', 'ols') # NULL or '_tracts'
