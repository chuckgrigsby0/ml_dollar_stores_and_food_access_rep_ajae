library(pacman)
p_load('furrr', 'fixest', 'xgboost', 'ranger', 'mlr3verse', 'mlr3tuningspaces', 'future', 'parallelly', 'tictoc', 'here') # 'doParallel'
...
class_ratios = class_ratios %>% filter(dep_var == model_dep_var) %>% pull(ratio); class_ratios
#--------------------------------------------------------------------------------------------#
...
search_space <- ps(
  ...
  # num.trees = p_dbl(lower = 1, upper = 500) 
)
