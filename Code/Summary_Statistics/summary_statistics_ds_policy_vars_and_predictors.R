...
# -------------------------------------------------------------------------------------------- #
# test <- standardized_mean_var(dta_arg = dta_model, var_name = 'inc_per_capita', group_var = 'policy_total_binary')
# -------------------------------------------------------------------------------------------- #
...
# -------------------------------------------------------------------------------------------- #
# Assumes common variance structure, therefore, uses a pooled sd. 
# -------------------------------------------------------------------------------------------- #
# mean_x <- mean(dta_model$inc_per_capita[dta_model$policy_total_binary == 1])
# mean_y <- mean(dta_model$inc_per_capita[dta_model$policy_total_binary == 0])
# 
# var_x <- var(dta_model$inc_per_capita[dta_model$policy_total_binary == 1])
# var_y <- var(dta_model$inc_per_capita[dta_model$policy_total_binary == 0])
# 
# l_x = sum(dta_model$policy_total_binary == 1) - 1 
# l_y = sum(dta_model$policy_total_binary == 0) - 1
# 
# csd <- l_x * var_x + l_y * var_y
# csd <- csd/(l_x + l_y)
# csd <- sqrt(csd)
# cd <- abs(mean_x - mean_y)/csd
# cd
# -------------------------------------------------------------------------------------------- #
# Unequal variance as in welch's t-test. 
# -------------------------------------------------------------------------------------------- #
# abs(mean_x - mean_y)/sqrt((var_x+var_y)/2)


# -------------------------------------------------------------------------------------------- #
# summary_stats <- dta_model %>% 
#   group_by(policy_total_binary) %>%
#   summarise(across(.cols = all_of(sel_modeling_vars), 
#                    .fns = list('mean' = mean, 'sd' = sd, 'var' = var), 
#                    .names = '{.col}_{.fn}'))
# 
# 
# last_col_pivot <- names(summary_stats)[length(names(summary_stats))]
# first_col_pivot <- names(summary_stats)[names(summary_stats) != 'policy_total_binary'][1]
# 
# 
# summary_stats <- summary_stats %>% 
#   pivot_longer(cols = all_of(first_col_pivot):all_of(last_col_pivot), 
#                names_pattern = '(.*)_(sd|mean|var)', 
#                names_to = c('variable', 'statistic'))  %>%
#   pivot_wider(names_from = policy_total_binary, values_from = value) %>%
#   left_join(sel_modeling_vars_table, by = 'variable') %>%
#   relocate(variable_clean, .after = variable) %>%
#   filter(variable %in% acs_covars)
