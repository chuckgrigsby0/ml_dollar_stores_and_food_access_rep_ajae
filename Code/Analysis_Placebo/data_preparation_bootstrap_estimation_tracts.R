...
# Load data. 
# -------------------------------------------------------------------------------------------- #
source(here::here('Code', 'Analysis_Placebo', 'load_data_for_imputation_estimation.R'))
# -------------------------------------------------------------------------------------------- #
...
non_model_vars <- c('DS_Count_10mile', 'DS_Count_10mile_diff',
                    'entry', 'entry_events', 'entry_events_bins', 'event_year', 
                    'net_entry_cumsum', 'net_entry_cumsum_bins', 
                    'rel_year', 'treat', 
                    'Grocery_Count_10mile', 'Grocery_Count_10mile_diff', 
...
dta_untreated <- dta_untreated %>% select(-c('entry_events_bins', 'net_entry_cumsum_bins'))
# -------------------------------------------------------------------------------------------- #
...
dta_treated <- dta_treated %>% select(-c('entry_events_bins', 'net_entry_cumsum_bins'))
...
# -------------------------------------------------------------------------------------------- #
geoids <- bind_rows(dta_untreated, dta_treated)
...
# This step removes the block groups treated in 2006, because
# in placebo models, this cohort is not used in model training. 
dta_untreated <- dta_untreated %>% filter(!is.na(event_year))
...
dta_treated <- dta_treated %>% filter(!is.na(GEOID))

dta_treated_non_model_vars <- dta_treated %>% 
  select(GEOID, year, any_of(non_model_vars), any_of(ends_with('bins') ))
