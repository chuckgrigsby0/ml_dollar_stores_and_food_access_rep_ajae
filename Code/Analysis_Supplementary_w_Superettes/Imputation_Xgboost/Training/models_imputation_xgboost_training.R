library('pacman')
p_load('here', 'furrr', 'fixest', 'xgboost', 'ParBayesianOptimization', 'future', 'parallelly', 'tictoc')
# -------------------------------------------------------------------------------------------- #  
...
# -------------------------------------------------------------------------------------------- #
filename = paste0('xgboost_10m_', str_to_lower(model_geography), '_', model_dep_var, '_training_w_superettes', '.rds')
saveRDS(xgb_cv, here::here('Analysis', 'Model_Training', dep_var_dir, filename))
# -------------------------------------------------------------------------------------------- #
# Use the xgb_cv object to estimate final cross-validated models using optimal hyperparameters; estimate a final model using all 
# untreated/yet-to-be treated data.  
# Apply optimal model to treated observations to predict counterfactuals of treated outcomes. 
# -------------------------------------------------------------------------------------------- #
gc()
# -------------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------------- #
...
toc()
# -------------------------------------------------------------------------------------------- #
filename = paste0('xgboost_10m_', str_to_lower(model_geography), '_', model_dep_var, '_final_w_superettes', '.rds')
