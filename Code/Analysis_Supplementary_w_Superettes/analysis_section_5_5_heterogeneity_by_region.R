...
posttre_effects_wdsentry <- posttre_effects %>%
  select(all_of(posttr_key_vars), actual, preds, tau, REGION_NAME) %>%
...
summary_stats_by_covar <- function(dta, covar_bins){
  ...
  sum_by_covar$stat <- sum_by_covar$stat %>% str_replace_all(pattern = c('actual_avg' = 'Low Access (Obs.)', 
...
sum_stats_df <- summary_stats_by_covar(dta = posttre_effects_winteracts, covar_bins = sel_covars)


sum_stats_df <- sum_stats_df %>% mutate(across(.cols = where(is.numeric), .fns = \(x) round(x, digits =3)))
