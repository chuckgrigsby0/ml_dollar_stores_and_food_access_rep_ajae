...
# ---------------------------------------------------------------------------------- #
paste0('The number of distinct ', model_geography, ' block groups with at least one restriction policy.') 
posttre_effects_w_policy_vars_binned %>% 
  filter(Restrictions_and_Defeated_binary == 1 & Defeated == 0) %>%
  distinct(GEOID) %>%
  count() %>% 
  ungroup() %>% 
  mutate(total_per_group = sum(n), 
         share_per_group = n/total_per_group)
# ---------------------------------------------------------------------------------- #
paste0('The number of distinct ', model_geography, ' block groups with at least one defeat.') 
posttre_effects_w_policy_vars_binned %>% 
  filter(Defeated_and_Restrictions_binary == 1 & Restrictions == 0) %>% 
  distinct(GEOID) %>%
  count() %>% 
  ungroup() %>% 
  mutate(total_per_group = sum(n), 
...
paste0('The number of distinct ', model_geography, ' block groups with at least one defeat and restriction policy.') 
posttre_effects_w_policy_vars_binned %>% 
  filter(Defeated_and_Restrictions_binary == 1 & Restrictions_and_Defeated_binary == 1) %>% 
  distinct(GEOID) %>%
  count() %>% 
  ungroup() %>% 
  mutate(total_per_group = sum(n), 
...
posttre_effects_w_policy_vars_binned %>% 
  ...
  mutate(total_per_group = sum(n), 
         share_per_group = n/total_per_group)
paste0('The number of distinct ', model_geography, ' block groups with at least one defeat/restriction.') 
# ---------------------------------------------------------------------------------- #

# ---------------------------------------------------------------------------------- #
paste0('The number and share of ', model_geography, ' block groups with at least one restriction policy.') 
posttre_effects_w_policy_vars_binned %>% 
  group_by(Restrictions_and_Defeated_binary) %>% 
  count() %>% 
  ungroup() %>% 
  mutate(total_per_group = sum(n), 
         share_per_group = n/total_per_group)
# ---------------------------------------------------------------------------------- #
paste0('The number of ', model_geography, ' block groups with at least one defeat.') 
posttre_effects_w_policy_vars_binned %>% 
  group_by(Defeated_and_Restrictions_binary) %>% 
  count() %>% 
  ungroup() %>% 
  mutate(total_per_group = sum(n), 
         share_per_group = n/total_per_group)
...
posttre_effects_w_policy_vars_binned %>% 
  ...
  mutate(total_per_group = sum(n), 
         share_per_group = n/total_per_group)
paste0('The number of ', model_geography, ' block groups with at least one defeat/restriction.') 
# ---------------------------------------------------------------------------------- #
