...
# -------------------------------------------------------------------------------------------- #
source(here::here('Code', 'Analysis_Supplementary_w_Superettes', 'load_data_for_analysis_plot_effects_on_interactions.R'))
# -------------------------------------------------------------------------------------------- #
# Plot estimates. 
...
# Regression of tau on covariate bins interacted with dollar store entries. 
# -------------------------------------------------------------------------------------------- #
...
# source(here::here('Code', 'Functions', 'Function_sort_complex_levels.R'))
# new_levels <- sort_complex_levels(new_levels)

current_levels <- levels(effects_on_ds_x_covars$entry)
current_levels <- current_levels[current_levels != "\u2264 -1"]
new_levels <- c("\u2264 -1", current_levels)

# levels(effects_on_ds_x_covars$entry)[levels(effects_on_ds_x_covars$entry) == "[-8,-1]"] <- '\u2264 -1'
...
map2(ds_entry_str, ds_entry_varname, function(.x, .y){ 
  ...
  map2(tidy_name_str, covariate_varname, function(.z, .w){ 
    ...
    ggsave(here::here('Analysis_Supplementary_w_Superettes', 'Figures', dir_dep_var, model_geography, paste0('effects_on_interactions', bootstrap_by_tracts), # Saves to directories and subdirectories.
                      figname), 
...
# -------------------------------------------------------------------------------------------- #
# Regression of tau on grocery store bins (2005) interacted with dollar store entries. 
# -------------------------------------------------------------------------------------------- #
...
# -------------------------------------------------------------------------------------------- #
map2(tidy_name_str, covariate_varname, function(.x, .y){
  
  map2(ds_entry_str, ds_entry_varname, function(.z, .w){ 
    
    if (.x == "Grocery Stores And Superettes (2005)"){ 
      clean_legend_title <- "Grocery Stores &\nSuperettes (2005)"
    } else { 
      clean_legend_title <- .x  
      }
  
  plot_ds_entry_on_grocery_interacts(dta = effects_on_ds_x_grocery, 
                                     ci_label_str = '99% CI', 
                                     ci_level = qnorm(1 - (0.01/2)),
                                     filter_covariate_type = c(ds_entry_tidy == .z & tidy_name == .x), 
                                     one_or_more_grocers = TRUE,
                                     y_axis_title = 'Average Treatment Effects', 
                                     x_axis_title = paste0(.z),
                                     legend_title_str = clean_legend_title,
                                     title_str = NULL, 
                                     subtitle_str = NULL, 
    ...
    figname <- paste0(str_to_lower(model_geography), '_', model_dep_var, '_effects_on_',
                      .w, '_x_', .y, '_one_or_more_grocers', bootstrap_by_tracts, '.pdf')
...
map2(tidy_name_str, covariate_varname, function(.x, .y){
  ...
  map2(ds_entry_str, ds_entry_varname, function(.z, .w){ 
    ...
    figname <- paste0(str_to_lower(model_geography), '_', model_dep_var, '_effects_on_',
                      .w, '_x_', .y, '_zero_or_more_grocers', bootstrap_by_tracts, '.pdf')
...
# -------------------------------------------------------------------------------------------- #
# To limit the y-axes when plotting zero grocery stores and dollar store effects, find the 95% and 5% values in the 
# distribution of effects. 

# ci_distributions <- effects_on_ds_x_grocery %>% 
#   
#   filter(grocery_stores %in% as.character(0, 4, 1)) %>%
#   
#   mutate(ci_upper = estimate + 2.576*bootstrap_sd, 
#          ci_lower = estimate - 2.576*bootstrap_sd) %>% 
#   
#   group_by(ds_entry_tidy, tidy_name) %>% 
#   
#   reframe(across(.cols = c(ci_upper, ci_lower), 
#                  .fns = \(x) quantile_df(.x = x, prob_bins = 0.05) ) ) %>%
#   ungroup()
# # -------------------------------------------------------------------------------------------- #
# # Plot and save. 
# # Includes the cases when year 2005 grocery stores are == 0. 
# # Limits the y-axis using coord_cartesian(xlim, ylim)
# # -------------------------------------------------------------------------------------------- #
# map2(ds_entry_str, ds_entry_varname, function(.x, .y){ 
#   
#   ci_upper_val <- ci_distributions %>% 
#     filter(ds_entry_tidy == .x & tidy_name == tidy_name_str) %>% 
#     dplyr::slice(n() - 2) %>%
#     pull(ci_upper) %>%
#     as.numeric()
#   
#   ci_lower_val <- ci_distributions %>% 
#     filter(ds_entry_tidy == .x & tidy_name == tidy_name_str) %>%
#     dplyr::slice(2) %>%
#     pull(ci_lower) %>%
#     as.numeric()
...
#                                      one_or_more_grocers = FALSE,
#                                      y_axis_title = expression(tau), 
#                                      x_axis_title = paste0(.x),
#                                      legend_title_str = tidy_name_str,
#                                      title_str = NULL, 
#                                      subtitle_str = NULL, 
#                                      decimal_place_y = 0.01, 
#                                      lower_ylim = ci_lower_val, 
#                                      upper_ylim = ci_upper_val, 
#                                      nbreaks = 14)
#   
#   figname <- paste0(str_to_lower(model_geography), '_', model_dep_var, '_effects_on_',
#                     .y, '_x_', covariate_varname, '_zero_or_more_grocers_coord_cartesian', bootstrap_by_tracts, '.png')
# 
#   ggsave(here::here('Analysis_Supplementary_w_Superettes', 'Figures', dir_dep_var, model_geography, paste0('effects_on_interactions', bootstrap_by_tracts), # Saves to directories and subdirectories.
#                     figname), width = 12, height = 8, unit = 'in')
# 
#   
# })
# -------------------------------------------------------------------------------------------- #