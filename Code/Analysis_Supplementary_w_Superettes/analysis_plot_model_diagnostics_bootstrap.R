...
boot_data <- seq(1, 499, 1) %>%
  ...
  map(function(.iter){ 
    ...
    load_bootstrap_errors_and_predictions_array_w_superettes(model_geography_str = model_geography, model_dep_var_str = model_dep_var, 
...
emp_predictions_on_relyear <- join_bootstrap_ses_to_emp_estimates(empirical_estimates_dta = emp_predictions_on_relyear, 
                                                             ...
                                                             join_vars = c('rel_year', 'Outcome'))
# -------------------------------------------------------------------------------------------- #
# Aside: Average MSE across model runs. 
# -------------------------------------------------------------------------------------------- #
...
# -------------------------------------------------------------------------------------------- # 
# Regress tau, the causal parameter of interest, on relative time since treatment. 
# -------------------------------------------------------------------------------------------- # 
...
emp_tau_on_relyear <- join_bootstrap_ses_to_emp_estimates(empirical_estimates_dta = emp_tau_on_relyear, 
                                                          ...
                                                          join_vars = c('rel_year', 'Outcome'))
# -------------------------------------------------------------------------------------------- # 
# Regress tau on relative time interacted with binned entry events. 
# -------------------------------------------------------------------------------------------- # 
# emp_tau_on_relyear_x_eventbins <- effects_on_relative_time_x_entry_bin(model_preds_dta = model_preds, 
#                                                                        national = FALSE, 
#                                                                        geography_str = model_geography)
# 
# emp_tau_on_relyear_x_eventbins <- emp_tau_on_relyear_x_eventbins %>% mutate(id = 0) # to join to bootstrap estimates. 
# 
# # Extract the element containing bootstrapped estimates from regressing tau on rel_year. 
# boot_data_tau_on_relyear_x_eventbins <- subset_errors_and_predictions(bootstrap_data = boot_data, 
#                                                           list_element_name = 'rel_year_x_entry_bin') # Must use regular expression. 
# 
# # Add empirical estimates to bootstrapped estimates to include in computation of standard errors. 
# boot_data_tau_on_relyear_x_eventbins <- bind_rows(emp_tau_on_relyear_x_eventbins, boot_data_tau_on_relyear_x_eventbins)
# 
# # Compute standard error. 
# boot_se_tau_on_relyear_x_eventbins <- compute_bootstrap_standard_errors(dta = boot_data_tau_on_relyear_x_eventbins, 
#                                                             group_vars = c('rel_year', 'entry_events', 'Outcome'))  
# 
# # Combine bootstrap standard errors with the empirical estimates. 
# emp_tau_on_relyear_x_eventbins <- join_bootstrap_ses_to_emp_estimates(empirical_estimates_dta = emp_tau_on_relyear_x_eventbins, 
#                                                           bootstrap_estimates_dta = boot_se_tau_on_relyear_x_eventbins, 
#                                                           join_vars = c('rel_year', 'entry_events', 'Outcome'))
# # -------------------------------------------------------------------------------------------- # 
...
empirical_estimates <- list('errors' = emp_errors_on_relyear, 
                            ...
                            #'tau_on_relyear_x_eventbin' = emp_tau_on_relyear_x_eventbins)
...
ggsave(here::here('Analysis_Supplementary_w_Superettes', 'Figures', dir_dep_var, model_geography, paste0('errors_and_preds', bootstrap_by_tracts), # Saves to directories and subdirectories. 
       ...
       width = 8, height = 6, unit = 'in', dpi = 600)
# -------------------------------------------------------------------------------------------- #
# plot_effects_on_relyear_by_entry_bin(dta = empirical_estimates$tau_on_relyear_x_eventbin, 
#                                      y_value = estimate, 
#                                      standard_error = bootstrap_sd,
#                                      y_axis_title = expression(tau), 
#                                      x_axis_title = 'Time from Treatment', 
#                                      # plot_title = 'Treatment Effects by Time from Treatment and Number of Entry Events',
#                                      # plot_subtitle = paste(paste('Geography:', model_geography), paste('Outcome:', dep_var_title), sep = '\n'),
#                                      plot_title = NULL, 
#                                      plot_subtitle = NULL,
#                                      #breaks_y = breaks_errors, 
#                                      decimal_place_y = 0.001, 
#                                      y1_lim = plot_coords$tau_on_relyear_x_eventbin$y_coord_lower,
#                                      y2_lim = plot_coords$tau_on_relyear_x_eventbin$y_coord_upper)
# 
...
#                   paste0('tau_vs_relative_time_x_eventbins_', str_to_lower(model_geography), '_', model_dep_var, bootstrap_by_tracts, '.png')), 
