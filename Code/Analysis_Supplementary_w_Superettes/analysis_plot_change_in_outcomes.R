...
# Save table. Commented out to avoid resizing tables (4/15/2025)
# write_csv(table_emp_change_in_outcomes, file = here::here('Analysis_Supplementary_w_Superettes', 
#                                                     'Tables', 
#                                                     dir_dep_var, 
...
# Save table. 
# write_csv(table_emp_change_in_outcomes_het, file = here::here('Analysis_Supplementary_w_Superettes', 
#                                                           'Tables', 
#                                                           dir_dep_var, 
#                                                           str_to_title(model_geography),
#                                                           paste0(str_to_lower(model_geography), '_' , 
...
# Save Figure.
ggsave(filename = here::here('Analysis_Supplementary_w_Superettes', 
                             'Figures', 
                             dir_dep_var, 
                             model_geography, 
...
ggsave(filename = here::here('Analysis_Supplementary_w_Superettes', 
       ...
       width = 8, height = 6, unit = 'in', dpi = 600)
# -------------------------------------------------------------------------------------------- #
# From the empirical/original data, compute the total change in tau across all periods. 
# posttre_effects <- model_preds %>% 
#   filter(rel_year >= 0) %>% 
#   filter(grepl(model_geography, Geography)) %>%
#   mutate(rel_year = factor(rel_year))
# 
# # Compute Net change over all periods. 
# emp_change_in_outcomes_sumnet <- posttre_effects %>% 
#   summarise(across(.cols = tau, .fns = c(sum = sum), .names = 'total_change_{.fn}')) %>%
#   mutate(effect_type = 'Net')
# # Compute Gross change over all periods. 
# emp_change_in_outcomes_sumgross <- posttre_effects %>% 
#   filter(tau == 1) %>%
#   summarise(across(.cols = tau, .fns = c(sum = sum), .names = 'total_change_{.fn}')) %>%
#   mutate(effect_type = 'Gross')
# 
# emp_change_in_outcomes_sum <- bind_rows(emp_change_in_outcomes_sumnet, 
#                                         emp_change_in_outcomes_sumgross)
# 
# # Using the bootstrap data, compute the totals by summing the totals for each bootsrap iteration. 
# # Then, compute the standard deviation for each total across the bootsrap models. 
# summary_boot_data_avg <- boot_data %>% 
#   group_by(effect_type, id) %>% 
#   summarise(across(.cols = total_change, .fns = c(sum = sum), .names = '{.col}_{.fn}')) %>%
#   arrange(effect_type) %>%
#   group_by(effect_type) %>%
#   summarise(across(.cols = total_change_sum, 
#                    .fns = c(mean = mean, sd = sd), 
#                    .names = '{.col}_{.fn}'))
# 
# # Join bootstrap data to the empirical/original data. 
# emp_change_in_outcomes_sum <- emp_change_in_outcomes_sum %>%
#   
#   left_join(summary_boot_data_avg, by = 'effect_type') %>%
#   
#   relocate(c(total_change_sum_sd, total_change_sum_mean), .after = total_change_sum) %>% 
#   
#   mutate(lower_ci_99 = total_change_sum - 2.576*total_change_sum_sd, 
#          upper_ci_99 = total_change_sum + 2.576*total_change_sum_sd, 
#          effect_type = paste(effect_type, 'Change')) %>%
#   
#   relocate(effect_type, .after = last_col())
# -------------------------------------------------------------------------------------------- #

# -------------------------------------------------------------------------------------------- #
# Save table. 
# write_csv(emp_change_in_outcomes_sum, file = here::here('Analysis_Supplementary_w_Superettes', 
#                                                    'Tables', 
#                                                    dir_dep_var, 
#                                                    str_to_title(model_geography),
#                                                    paste0(str_to_lower(model_geography), '_' , 
#                                                           'average_change_in_outcomes', '.csv')))
