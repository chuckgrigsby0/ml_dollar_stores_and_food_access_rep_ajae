...
bootstrap_errors_and_preds <- function(bootstrap_ids, iter, bootstrap_by_tracts){ 
  ...
  treated_preds <- model_output$data_cf_preds %>%
    ...
    filter(year >= '2005') # Counterfactual predictions are made for years 2007-2020 in the figures for consistency with CV error preds. 
                            # Post-treatment placebo effects are assessed from 2006-2020.
  # -------------------------------------------------------------------------------------------- #
